...
export function CoffeeStorm({
  ...
  useEffect(() => {
    ...
    if (
      ...
      preference.matches ||
      document.hidden ||
      (!force && (window.scrollY > 30 || window.location.hash))
    )
      return;

    if (introSeen && !force) return;
    ...
    const onKey = (event: KeyboardEvent) => {
      if (["Tab", "Escape", "ArrowDown", "PageDown", " "].includes(event.key))
        finish();
    };
  ...
  return (
    <div ref={layer} className="coffee-storm" hidden>
      <canvas
        ref={canvas}
        className="coffee-storm__canvas"
        aria-hidden="true"
      />
      ...
      <div className="coffee-storm__timeline" aria-hidden="true">
        <span />
      </div>
    </div>
  );
