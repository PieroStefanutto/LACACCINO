...
try {
  ...
  const profile = fields(page.html, 'id="first-name"'); profile.set("first_name", "HTTP"); profile.set("last_name", "Test"); profile.set("phone", "");
  await request("/konto", profile);
  assert.equal((await admin.from("profiles").select("first_name").eq("id", userId).single()).data?.first_name, "HTTP", "profile action persists");
  page = await request("/konto");
  const join = fields(page.html, 'Auf die Warteliste setzen'); join.set("intent", "join"); join.set("consent", "yes");
  ...
  page = await request("/konto");
  const leave = fields(page.html, 'Von der Warteliste abmelden'); leave.set("intent", "leave");
